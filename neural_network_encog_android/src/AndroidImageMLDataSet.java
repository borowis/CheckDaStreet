 ...
 * @author jheaton
 */
public class AndroidImageMLDataSet extends BasicMLDataSet {
     ...
     *            The low value to normalize to.
     */
    public AndroidImageMLDataSet (final Downsample theDownsampler,
    ...
    public final void downsample(final int theHeight, final int theWidth) {
        ...
        for (final MLDataPair pair : this) {
            if (!(pair.getInput() instanceof AndroidImageMLData)) {
                throw new NeuralNetworkError(
                        "Invalid class type found in ImageNeuralDataSet, only "
                                + "ImageNeuralData items are allowed.");
            }

            final AndroidImageMLData input = (AndroidImageMLData) pair.getInput();
